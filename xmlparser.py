...
class XmlParser:
    ...
    def __parse_channels(self):
        ...
        for channel in self.__root.findall('channel'):
            ...
            result = self.__database.start((channel_id, title, lang, icon))
            if not result:
                error(self.__database.error())
            info("Channel {} {} added.".format(channel_id, title.encode('utf-8')))
    ...
    def __parse_programme(self):
        ...
        for programme in self.__root.findall('programme'):
            ...
            for category_id in categories:
                result = self.__database.start((pid, category_id))
                if not result:
                    error(self.__database.error())
                    continue
    ...
    def __parse_categories(self, categories):
        ...
        for category in categories:
            if category.text not in self.__db_categories.values():
                ...
                if not result:
                    error(self.__database.error())
                info('Add category {} to database, id {}'.format(category.text.encode('utf-8'), self.__database.last_insert_id()))
